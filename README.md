...
| `История и статистика…` | Таблица за 7 дней с балансом и графики за неделю, месяц или год. Открывается и двойным щелчком по значку. |
| `Настройки…` | Учёт времени, график, уведомления, оформление. |
...
Одно окно с двумя вкладками: «История за неделю» — таблица последних семи дней, в которой
день можно починить, и «Статистика» — графики за период. Открывается из меню трея и двойным
щелчком по значку.
...
### Числа рядом с графиком

Под графиком всегда видно:

- отработано за период, норма периода и норма к сегодняшнему дню — у незакончившегося периода
  это разные числа, и сравнивать отработанное с полной нормой месяца первого числа бессмысленно;
- накопленный баланс — разница по уже прожитым дням;
- во сколько обычно приходите и уходите, средняя продолжительность дня, средняя длительность
  обеда и сколько дней в периоде отработано.
...
Кнопка «Выгрузить в CSV» сохраняет период по дням: дата, день недели, приход, уход, отработанное,
...
Нечитаемый файл дня статистику не роняет: такой день пропускается, а под графиком появляется
...
ни то, ни другое не меняет подсчёта времени — они выбирают только то, что видно и когда сказано.
