...
if ($running.Count -gt 0) {
    ...
    foreach ($process in $running) {
        # Убивать безопасно: отметки прихода, ухода и пауз ложатся в файл дня сразу,
        # накопленное за день не теряется. Отстать может только метка живучести —
        # она пишется реже, и на подсчёте отработанного это не сказывается.
